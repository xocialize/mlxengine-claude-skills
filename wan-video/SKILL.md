...
## Streamed weights — `BlockStreamer` (wan-core v0.1.0, the weight-side plane)

The levers above bound ACTIVATIONS; `BlockStreamer` bounds the DiT **weights** — per-block SSD streaming through two slots, opt-in per consumer config, **bit-exact** (memcmp vs resident, with determinism + poisoned-slot controls; receipts `mlxengine-todo/probes/hv2_wan_blockstreamer.out`, S7 consumer gate on bernini-r `d02cfa1`).

- **Granules:** `wan-granule-layout` CLI → per-block contiguous 16 KiB-aligned files; shared store `/Volumes/Satechi/Models/wan-granules/<quant>/` (A14B bf16+int4 already laid out, ~67 GB, reusable). Manifest provenance check refuses stale/wrong-quant trees.
- **Self-calibrating gate** `N ≥ B·F/(2·S)` (S from the first refill sweep, F in-regime during step 1) with AUTOMATIC output-invisible resident fallback. Image-class N falls back on fast machines (N_min≈6.7–7.2k cool-regime, conservative); video-class N streams (A14B N=15600: **0.0% stall**, ×1.115 vs in-regime resident; 17 GB int4 dual-expert ran 480p/17f CFG in **7.11 GB** MLX peak under a 12.03 GB cap). The A14B t=875 expert switch just re-activates IO on the other granule set. After an `.auto` fallback the streamer must be RELEASED (else 0.74–2.62 GiB of slots strand).
- 🚨 **THE TRAP — a bound streamer silently corrupts any HAND-WRITTEN block loop.** Anything iterating `model.blocks` directly instead of `runBlocks`/`prepareCrossKV` reads unrefilled slots → plausible-looking garbage, NOT a crash (bernini's `forwardMultiseg` editing surfaces r2v/v2v/rv2v now REFUSE while streamed; streaming them needs a public group-window API + its own IO receipt). **Audit every consumer's block loops before allowing its config to stream.**
- **Footprint honesty:** the 7.11 GB receipt is DiT-denoise-phase ONLY (umT5 ~22 GB fp32 encode and VAE are out of scope) — do NOT declare it as `residentBytes`; the whole-pipeline app-seam re-measure gates that and the §5.6 bandwidth field (NEUROSTREAM-ACTIONS HV2).
- **Harness lessons:** looped fallbacks ratchet `phys_footprint` (~52 GiB/rung never reclaimed in-process → run one rung per process); `| tee` masks signal kills (run receipt binaries bare); thermal drift slows all large-N compute ~2× after sustained load (150 s idle recovers) — calibrate F and compare arms in-regime.
