...
| 2026-08-30 | SCUNet (17.9M, Swin-Conv UNet) | window attention | **CoreAI-GPU or MLX — NOT the ANE** | MEASURED: converts fine (peak RSS 1.05 GB), but rank-6 from `einops.rearrange` → 678 ANE rejections → **ANE 30× SLOWER than the GPU lane** (249.3 vs 8.2 ms) and 37.78 vs 71.22 dB. → `case-scunet-window-attention.md` |
