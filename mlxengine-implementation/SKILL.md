...
0. **Adopt the design vocabulary BEFORE the first view.** `DesignScaffold` is the fleet's single
   design authority (AB-D-0042): add the package, build every surface from `Tokens`, and check
   **`DesignScaffold/Docs/COMPONENTS.md`** — the generated catalog — before writing any UI a shared
   component may already cover (loading modal, run stepper, calendar, playlist). Retrofitting a token
   layer means touching every view you already wrote. **No view hardcodes a colour, font size, or
   spacing value**; a missing value gets added to `Tokens` by ask, not invented locally. See
   [references/design-vocabulary.md](references/design-vocabulary.md).
...
6. **Reuse the settings UI** — `EngineSettingsView(storage:)` already gives you the storage panel (disk
   used / models installed / free space) and web-retrieval settings. Don't rebuild it. Engine-management
   panels come from `MLXEngineUI`; everything else you build from `Tokens` (step 0). Caveat while it
   lasts: those panels still carry `MarqueeTokens` (hardcoded hex), so they read as a dark island in
   light mode — conformance requested in AB-A-0019.
...
| 12 | **Design vocabulary & shared components** (DesignScaffold as the single design authority; the generated component catalog to check BEFORE writing UI; the token house rule; `LoadingCard` / `StageStepper` for engine-shaped progress; how to request a component and the intake bar; the MarqueeTokens seam pending AB-A-0019) | ✅ [references/design-vocabulary.md](references/design-vocabulary.md) |
